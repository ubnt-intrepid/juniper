...
impl Query<Context = ()> {
    ...
    fn doc(&self) -> DocComment {
      DocComment{
        regular_field: true,
      }
    }

    fn multi_doc(&self) -> MultiDocComment {
      MultiDocComment{
        regular_field: true,
      }
    }

    fn override_doc(&self) -> OverrideDocComment {
      OverrideDocComment{
        regular_field: true,
      }
    }

    fn skipped_field_obj(&self) -> SkippedFieldObj {
        SkippedFieldObj{
            regular_field: false,
            skipped: 42,
        }
